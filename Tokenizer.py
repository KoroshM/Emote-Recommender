# This tokenizer is originally from the Emote-Controlled study by Kobs et al.
# It was posted to the study's repository on GitHub at: https://github.com/konstantinkobs/emote-controlled
# The tokenize() function was slightly modified to accept a list of emotes not to process within the input text.
# This functionality is used exclusively by the recommender system, and was not used for preprocessing the training data.
#
# -Korosh Moosavi 2023
...
emoji_lexicon = emoji.get_emoji_unicode_dict
...
def load_labeled_emotes():
    emotes = set(pd.read_table("E:\Personal\Random Projects\Thesis\Downloads\EmoteControlled\lexica\emote_average.tsv")["word"])
    return emotes
...
class TwitchTokenizer:
    ...
    def tokenize(self, text, emotes=load_labeled_emotes()):
        """
        :param text: str
        :param emotes: list
        :rtype: list(str)
        ...
        token_list = []
        for word in words:
            if word not in emotes and not re.match(EMOTICON_RE,word):
                ...
                word = re.sub(SHORTENING_OF, r'\1\1\1', word)
            token_list.append(word)

        # remove all non-alphabetical characters, keep line emoticons, unicode-emoji & emotes
        def keep_token(token):
            if re.match(NON_ALPHABETICAL,token) and not re.match(EMOTICON_RE,token) and token not in emotes \
